...
fffffffff]]
    ...
    local function fg(l)
        if term.isColor() then
            return string.rep("4", l)
        else
            return string.rep("8", l)
        end
    end

    local function text(title, str)
        return {title .. str, fg(string.len(title)) .. string.rep("0", string.len(str)), string.rep("f", string.len(title .. str))}
    end
    ...
    local function ext(retval)
        if debug ~= nil then
            table.insert(retval, text("    ", "Debug enabled"))
        end

        if http ~= nil then
            table.insert(retval, text("    ", "HTTP enabled"))

            if http.websocket ~= nil then
                table.insert(retval, text("    ", "CC: Tweaked"))
            end
        end

        if mounter ~= nil then
            table.insert(retval, text("    ", "CraftOS-PC"))
        end

        if term.setGraphicsMode ~= nil then
            table.insert(retval, text("    ", "CraftOS-PC GFX"))
        end

        if term.screenshot ~= nil then
            table.insert(retval, text("    ", "CraftOS-PC 2"))
        end

        if ccemux ~= nil then
            table.insert(retval, text("    ", "CCEmuX"))
        end

        if fs.exists(".mbs") or fs.exists("rom/.mbs") then
            table.insert(retval, text("    ", "MBS"))
        end

        if type(kernel) == "table" then
            table.insert(retval, text("    ", "CCKernel2"))
        end
    ...
    local function getRuntime()
        if os.about ~= nil then
            return string.sub(os.about(), 1, string.find(os.about(), "\n"))
        elseif ccemux ~= nil then
            return ccemux.getVersion()
        elseif _MC_VERSION ~= nil then
            return _MC_VERSION
        else
            return "Unknown"
        end
    ...
    local sysinfo = {text(os.getComputerLabel() or "Untitled Computer", ""), text("Type: ", commands ~= nil and "Command Computer" or term.isColor() and "Advanced Computer" or "Standard Computer"), text("Main OS: ", main), text("Co-OS: ", co), text("Runtime: ", getRuntime()), text("Lua: ", _VERSION), text("Host: ", _HOST), text("Uptime: ", time(os.clock())), text("Size: ", fs.attributes2("/").size), text("COS API: ", cosUtils.getSizeOfCosUtils() .. " functions loaded"), text("Extensions: ", "")}
    ...
    for i = 1, string.len(ccart), ccart_width do
        ...
        if sysinfo[((i - 1) / ccart_width) + 1] ~= nil then
            term.blit(table.unpack(sysinfo[((i - 1) / ccart_width) + 1]))
        end
...
ffffffffffffffffffffffff]]
    ...
    local function mem(bytes)
        if bytes > 1048576 then
            return string.gsub(string.sub((math.floor((bytes / 1048576) * 1000) / 1000) .. "", 1, 4), "%.$", "") .. " MB"
        elseif bytes > 1024 then
            return string.gsub(string.sub((math.floor((bytes / 1024) * 1000) / 1000) .. "", 1, 4), "%.$", "") .. " kB"
        else
            return bytes .. " B"
        end
    end
    ...
    local function ext(retval)
        for address, type in component.list() do
            table.insert(retval, text("    " .. type .. " ", address))
        end
    ...
    for k, v in pairs(component.computer.getDeviceInfo()) do
        if v.class == "processor" then
            tier = string.match(v.product, "FlexiArch (%d+) Processor")
        end
    end

    local sysinfo = {text("OpenComputers", ""), text("Tier: ", tier), text("OS: ", _OSVERSION), text("Lua: ", _VERSION), text("RAM: ", mem(computer.totalMemory() - computer.freeMemory()) .. " / " .. mem(computer.totalMemory())), text("Uptime: ", time(computer.uptime())), text("Components: ", "")}
    ...
    local function blit(text, fg, bg)
        ...
        if #text ~= #fg or #fg ~= #bg then
            error("Unbalanced string lengths in blit", 2)
        end
        ...
        for i = 1, #text do
            if ccColors[string.sub(fg, i, i)] == nil then
                error("Unknown color " .. string.sub(fg, i, i))
            end

            if ccColors[string.sub(bg, i, i)] == nil then
                error("Unknown color " .. string.sub(bg, i, i))
            end
    ...
    for i = 1, string.len(ccart), 25 do
        ...
        if sysinfo[((i - 1) / 25) + 1] ~= nil then
            blit(table.unpack(sysinfo[((i - 1) / 25) + 1]))
        end
