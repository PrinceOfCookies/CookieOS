...
local function sendFormattedChat(buildWhoisParts(targetName, shownClearance, targetAuth))
  local parts = buildWhoisParts(targetName, shownClearance, targetAuth)
  local nearbyPlayers = getNearbyPlayers()
  local sentAny = false

  -- Try targeted messages only for viewers with auth.whois.
  -- If AP targeted messages fail, fallback to radius broadcast.
  for _, viewer in ipairs(nearbyPlayers) do
    local allowed = nodeClient.authCheck(viewer, "auth.whois", 2.5)

    if allowed and sendFormattedToPlayer(viewer, parts) then
      sentAny = true
    end
  end
...
local function handleChatCommand(username, message)
  ...
  elseif command == "help" then
    sendFormattedChat({
      ...
      { text = "\n!where <name>", color = "gray" },
      { text = "\n!whois <name>", color = "gray" },
  
      { text = "\n\nSecurity", color = "yellow", bold = true },
      { text = "\n!sec <green/yellow/red/black>", color = "gray" },
  
      { text = "\n\nMaintenance", color = "yellow", bold = true },
      { text = "\n!status / !maint", color = "gray" },
      { text = "\n!nodes", color = "gray" },
      { text = "\n!offline", color = "gray" },
      { text = "\n!node <name>", color = "gray" },
      { text = "\n!services", color = "gray" },
    })
  end
